...
inline void multiply(const matrix::StaticMatrix<T, M, K>& A, const matrix::StaticMatrix<T, K, N>& B,
    ...
    for (std::size_t j = 0; j < N; ++j) {
        for (std::size_t k = 0; k < K; ++k) {
            ...
            if constexpr (std::is_same_v<T, float>) {
                ...
                for (; i + 3 < M; i += 4) {
                    vst1q_f32(&c_col[i],
                              vfmaq_f32(vld1q_f32(&c_col[i]), vld1q_f32(&a_col[i]), v_b));
                }
            } else if constexpr (std::is_same_v<T, double>) {
                float64x2_t v_b = vdupq_n_f64(b_val);
                for (; i + 1 < M; i += 2) {
                    vst1q_f64(&c_col[i],
                              vfmaq_f64(vld1q_f64(&c_col[i]), vld1q_f64(&a_col[i]), v_b));
                }
            }
#elif !defined(__CUDACC__) && defined(__AVX2__)
            if constexpr (std::is_same_v<T, float>) {
                ...
                for (; i + 7 < M; i += 8) {
                    _mm256_storeu_ps(&c_col[i], _mm256_fmadd_ps(_mm256_loadu_ps(&a_col[i]), v_b,
                                                                _mm256_loadu_ps(&c_col[i])));
                }
            } else if constexpr (std::is_same_v<T, double>) {
                __m256d v_b = _mm256_set1_pd(b_val);
                for (; i + 3 < M; i += 4) {
                    _mm256_storeu_pd(&c_col[i], _mm256_fmadd_pd(_mm256_loadu_pd(&a_col[i]), v_b,
                                                                _mm256_loadu_pd(&c_col[i])));
                }
            }
#endif
            for (; i < M; ++i) c_col[i] += a_col[i] * b_val;
        }
    }
}

template <typename T, std::size_t M, std::size_t K, std::size_t N>
inline void multiply_AT_B(const matrix::StaticMatrix<T, K, M>& A, const matrix::StaticMatrix<T, K, N>& B,
                          matrix::StaticMatrix<T, M, N>& C) noexcept {
    const T* a_ptr = A.data_ptr();
    const T* b_ptr = B.data_ptr();
    T* c_ptr = C.data_ptr();
    ...
    for (std::size_t j = 0; j < N; ++j) {
        for (std::size_t i = 0; i < M; ++i) {
            ...
            if constexpr (std::is_same_v<T, float>) {
                ...
                for (; k + 3 < K; k += 4) {
                    v_sum = vfmaq_f32(v_sum, vld1q_f32(&a_col[k]), vld1q_f32(&b_col[k]));
                }
            ...
            if constexpr (std::is_same_v<T, float>) {
                ...
                for (; k + 7 < K; k += 8) {
                    v_sum = _mm256_fmadd_ps(_mm256_loadu_ps(&a_col[k]), _mm256_loadu_ps(&b_col[k]),
                                            v_sum);
                }
                ...
                for (; k + 3 < K; k += 4) {
                    v_sum = _mm256_fmadd_pd(_mm256_loadu_pd(&a_col[k]), _mm256_loadu_pd(&b_col[k]),
                                            v_sum);
                }
...
template <typename T, std::size_t M, std::size_t K, std::size_t N>
inline matrix::StaticMatrix<T, M, N> operator*(const matrix::StaticMatrix<T, M, K>& A,
                                       const matrix::StaticMatrix<T, K, N>& B) noexcept {
...
inline matrix::StaticMatrix<T, M, N> operator*(T scalar, const matrix::StaticMatrix<T, M, N>& A) noexcept {
    ...
    for (std::size_t i = 0; i < size; ++i) C(i) *= scalar;
    return C;
}

template <typename T, std::size_t M, std::size_t N>
inline matrix::StaticMatrix<T, M, N> operator*(const matrix::StaticMatrix<T, M, N>& A, T scalar) noexcept {
    return scalar * A;
}

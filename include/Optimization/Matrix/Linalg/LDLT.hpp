    ...
    namespace linalg {
        ...
        inline MathStatus LDLT_decompose(MatType& mat) noexcept {
            ...
            for (std::size_t j = 0; j < N; ++j) {
                ...
                T d_jj = mat(j, j);
                // NaN 침투 및 극소 피벗 (Pivot)에 의한 행렬 발산 방지
                if (std::isnan(d_jj) || std::abs(d_jj) <= std::numeric_limits<T>::epsilon()) {
        ...
        /**
         * @brief SIMD-Accelerated Solver
         */
        template <typename MatType, typename VecB, typename VecX>
                ...
                std::size_t i = k + 1;
                    ...
                    if constexpr (std::is_same_v<T, float>) {
                        ...
                        for (; i + 7 < N; i += 8) {
                            ...
                            __m256 v_x = _mm256_loudu_ps(&x(i));
                            _mm256_storeu_ps(&x(i), _m256_fnmadd_ps(v_mat, v_xk, v_x));
                        }
                    } else if constexpr (std::is_same_v<T, double>) {
            ...
            for (int i = static_cast<int>(N) - 1; i >= 0; --i) {
                ...
                std::size_t k = i + 1;
                    ...
                    if constexpr (std::is_same_v<T, float>) {
                        ...
                        __m128 v_low = _mm256_castpd256_pd128(v_sum);
                        __m128 v_high = _mm256_extractf128_pd(v_sum, 1);

...
/**
 * @brief [Architect's Update] MGS(Modified Gram-Schmidt) QR 분해 (100% SIMD & noexcept)
 */
template <class MatType, class MatR>
    ...
    for (std::size_t i = 0; i < Cols; ++i) {
...
#else
            norm_sq += vgetq_lane_f32(v_sum, 0) + vgetq_lane_f32(v_sum, 1) + 
        ...
        for (std::size_t j = i + 1; j < Cols; ++j) {
...
#else
                dot += vgetq_lane_f32(v_dot, 0) + vgetq_lane_f32(v_dot, 1) + 
            ...
            if constexpr (std::is_same_v<T, float>) {
                ...
                for (; k + 7 < Rows; k += 8)
                    v_dot = _mm256_fmadd_ps(_mm256_loadu_ps(&mat(k, i)),
                                            _mm256_loadu_ps(&mat(k, j)), v_dot);
                ...
                for (; k + 3 < Rows; k += 4)
                    v_dot = _mm256_fmadd_pd(_mm256_loadu_pd(&mat(k, i)),
                                            _mm256_loadu_pd(&mat(k, j)), v_dot);
            ...
            if constexpr (std::is_same_v<T, float>) {
                ...
                for (; k + 3 < Rows; k += 4)
                    vst1q_f32(&mat(k, j),
                              vfmaq_f32(vld1q_f32(&mat(k, j)), vld1q_f32(&mat(k, i)), v_neg_dot));
            } else if constexpr (std::is_same_v<T, double>) {
                float64x2_t v_neg_dot = vdupq_n_f64(neg_dot);
                for (; k + 1 < Rows; k += 2)
                    vst1q_f64(&mat(k, j),
                              vfmaq_f64(vld1q_f64(&mat(k, j)), vld1q_f64(&mat(k, i)), v_neg_dot));
            }
#elif !defined(__CUDACC__) && defined(__AVX2__)
            if constexpr (std::is_same_v<T, float>) {
                __m256 v_neg_dot = _mm256_set1_ps(neg_dot);
                for (; k + 7 < Rows; k += 8)
                    _mm256_storeu_ps(&mat(k, j),
                                     _mm256_fmadd_ps(_mm256_loadu_ps(&mat(k, i)), v_neg_dot,
                                                     _mm256_loadu_ps(&mat(k, j))));
            } else if constexpr (std::is_same_v<T, double>) {
                __m256d v_neg_dot = _mm256_set1_pd(neg_dot);
                for (; k + 3 < Rows; k += 4)
                    _mm256_storeu_pd(&mat(k, j),
                                     _mm256_fmadd_pd(_mm256_loadu_pd(&mat(k, i)), v_neg_dot,
                                                     _mm256_loadu_pd(&mat(k, j))));
            }
#endif
...
/**
 * @brief [Architect's Update] Householder QR 분해 (SIMD Dot-Product 강화)
 */
...
inline MathStatus QR_decompose_Householder(MatType& mat, VecTau& tau) noexcept {
    ...
    for (std::size_t i = 0; i < Cols; ++i) {
        ...
        for (k = i + 1; k < Rows; ++k) mat(k, i) /= v0;

        T v_sq_norm = static_cast<T>(1.0);
        for (k = i + 1; k < Rows; ++k) v_sq_norm += mat(k, i) * mat(k, i);
        ...
        for (std::size_t j = i + 1; j < Cols; ++j) {
            ...
            if constexpr (std::is_same_v<T, float>) {
                ...
                for (; k + 3 < Rows; k += 4)
                    vst1q_f32(&mat(k, j),
                              vfmaq_f32(vld1q_f32(&mat(k, j)), vld1q_f32(&mat(k, i)), v_neg_tau));
            } else if constexpr (std::is_same_v<T, double>) {
                float64x2_t v_neg_tau = vdupq_n_f64(neg_tau_dot);
                for (; k + 1 < Rows; k += 2)
                    vst1q_f64(&mat(k, j),
                              vfmaq_f64(vld1q_f64(&mat(k, j)), vld1q_f64(&mat(k, i)), v_neg_tau));
            }
#elif !defined(__CUDACC__) && defined(__AVX2__)
            if constexpr (std::is_same_v<T, float>) {
                ...
                for (; k + 7 < Rows; k += 8)
                    _mm256_storeu_ps(&mat(k, j),
                                     _mm256_fmadd_ps(_mm256_loadu_ps(&mat(k, i)), v_neg_tau,
                                                     _mm256_loadu_ps(&mat(k, j))));
            } else if constexpr (std::is_same_v<T, double>) {
                __m256d v_neg_tau = _mm256_set1_pd(neg_tau_dot);
                for (; k + 3 < Rows; k += 4)
                    _mm256_storeu_pd(&mat(k, j),
                                     _mm256_fmadd_pd(_mm256_loadu_pd(&mat(k, i)), v_neg_tau,
                                                     _mm256_loadu_pd(&mat(k, j))));
            }
#endif
...
/**
 * @brief [Architect's Update] Householder 반사 적용 및 SIMD 역대입
 */
template <class MatType, class VecTau, class VecB, class VecX>
inline void QR_solve_Householder(const MatType& mat, const VecTau& tau, const VecB& b,
                                 VecX& x) noexcept {
    using T = typename std::remove_const<typename MatType::value_type>::type;
    constexpr std::size_t Rows = MatType::NumRows;
    constexpr std::size_t Cols = MatType::NumCols;
    ...
    for (std::size_t i = 0; i < Rows; ++i) y(i) = b(i);
    ...
    for (std::size_t i = 0; i < Cols; ++i) {
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            float64x2_t v_neg_tau = vdupq_n_f64(neg_tau_dot);
            for (; k + 1 < Rows; k += 2)
                vst1q_f64(&y(k), vfmaq_f64(vld1q_f64(&y(k)), vld1q_f64(&mat(k, i)), v_neg_tau));
        }
#elif !defined(__CUDACC__) && defined(__AVX2__)
        if constexpr (std::is_same_v<T, float>) {
            __m256 v_neg_tau = _mm256_set1_ps(neg_tau_dot);
            for (; k + 7 < Rows; k += 8)
                _mm256_storeu_ps(&y(k), _mm256_fmadd_ps(_mm256_loadu_ps(&mat(k, i)), v_neg_tau,
                                                        _mm256_loadu_ps(&y(k))));
        } else if constexpr (std::is_same_v<T, double>) {
            __m256d v_neg_tau = _mm256_set1_pd(neg_tau_dot);
            for (; k + 3 < Rows; k += 4)
                _mm256_storeu_pd(&y(k), _mm256_fmadd_pd(_mm256_loadu_pd(&mat(k, i)), v_neg_tau,
                                                        _mm256_loadu_pd(&y(k))));
        }
#endif
        for (; k < Rows; ++k) y(k) += neg_tau_dot * mat(k, i);
    }

    // 1. 결과 벡터 x에 R * x = y_1:Cols 풀이를 위한 상단 복사
    for (std::size_t i = 0; i < Cols; ++i) x(i) = y(i);
    ...
    for (int k = static_cast<int>(Cols) - 1; k >= 0; --k) {
        ...
        if constexpr (std::is_same_v<T, float>) {
            ...
            for (; i + 7 < k; i += 8)
                _mm256_storeu_ps(&x(i), _mm256_fnmadd_ps(_mm256_loadu_ps(&mat(i, k)), v_xk,
                                                         _mm256_loadu_ps(&x(i))));
        } else if constexpr (std::is_same_v<T, double>) {
            __m256d v_xk = _mm256_set1_pd(x_k);
            for (; i + 3 < k; i += 4)
                _mm256_storeu_pd(&x(i), _mm256_fnmadd_pd(_mm256_loadu_pd(&mat(i, k)), v_xk,
                                                         _mm256_loadu_pd(&x(i))));
        }
#endif
        for (; i < k; ++i) x(i) -= mat(i, k) * x_k;
    }
}

template <class MatType, class VecTau, class VecB>
inline matrix::StaticVector<typename std::remove_const<typename MatType::value_type>::type,
                    MatType::NumCols>
